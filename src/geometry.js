...
export async function parseOBJFile(url) {
    ...
    for (const line of text.split('\n')) {
        ...
        if (tokens[0] === 'v')
            rawPositions.push([+tokens[1], +tokens[2], +tokens[3]]);
        if (tokens[0] === 'vt')
            rawTexCoords.push([+tokens[1], 1 - +tokens[2]]);
        if (tokens[0] === 'vn')
            rawNormals.push([+tokens[1], +tokens[2], +tokens[3]]);
        if (tokens[0] === 'f') {
...
export function buildSphereMesh(radius = 1, rows = 32, cols = 32) {
    const positions = [];
    const normals = [];
    const uvs = [];
    const indices = [];
    ...
    return new GeometryBuffer(new Float32Array(positions), new Float32Array(normals), new Float32Array(uvs), new Uint32Array(indices));
}
export function buildCubeMesh(size = 1) {
    const h = size / 2;
    const faces = [
