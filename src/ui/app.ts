import { SceneRenderer, SceneNode, quatMultiply } from '../core/scene_renderer';
import '../ui/theme.css';
...
function projectToSphere(x: number, y: number): [number,number,number] {
  const d2 = x*x + y*y;
  if (d2 <= 1.0) {
...
function quatFromVectors(
  ...
  const q: [number,number,number,number] = [dot, axis[0], axis[1], axis[2]];
  // Normalise
...
function toTrackballCoords(px: number, py: number): [number,number] {
...
let dragging  = false;
let lastX     = 0, lastY = 0;
let trackStart: [number,number,number] | null = null;
...
function buildSliders(containerId: string, defs: typeof transformDefs) {
  ...
  for (const def of defs) {
    ...
    row.querySelector('input')!.addEventListener('input', e => {
      const node = getActive(); if (!node) return;
      const v = parseFloat((e.target as HTMLInputElement).value);
      (node as any)[def.key] = v;
...
function refreshPanel() {
  ...
  [...transformDefs, ...materialDefs].forEach(def => {
    ...
    if (inp && val) {
      inp.value       = String((node as any)[def.key]);
      val.textContent = Number((node as any)[def.key]).toFixed(3);
    }
  });
  ...
  if (utx) utx.checked = node.useTexture;
}

function refreshAll() { refreshNodeList(); refreshPanel(); }
function getActive(): SceneNode | null {
...
document.getElementById('btnSphere')!.addEventListener('click', () => {
  ...
  renderer.activeId = node.id; refreshAll();
});
...
document.getElementById('btnDeselect')!.addEventListener('click', () => {
  renderer.activeId = null; refreshAll();
});
document.getElementById('btnRemove')!.addEventListener('click', () => {
  if (renderer.activeId === null) return;
...
document.getElementById('texFile')!.addEventListener('change', async e => {
  ...
  const node = getActive(); if (!file || !node) return;
