...
function buildSliderGroup(containerId, props) {
    ...
    for (const prop of props) {
      ...
      <span class="slider-val" data-val="${prop.key}">0</span>`;
        ...
        row.querySelector('input').addEventListener('input', e => {
            ...
            node[prop.key] = v;
            row.querySelector(`[data-val="${prop.key}"]`).textContent = v.toFixed(3);
        });
    }
...
function refreshPanel() {
    ...
    [...transformProps, ...materialProps].forEach(prop => {
        const inp = document.querySelector(`input[data-key="${prop.key}"]`);
        const val = document.querySelector(`[data-val="${prop.key}"]`);
        if (inp && val) {
            inp.value = String(node[prop.key]);
            val.textContent = Number(node[prop.key]).toFixed(3);
        }
    });
    ...
    const tog = document.getElementById('useTexToggle');
    if (tog)
        tog.checked = node.useTexture;
}
